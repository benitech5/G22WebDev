...
// POST /api/auth/register — legacy signup route alias for practitioner registration
server.post('/api/auth/register', (req, res) => {
  const { username, email, password } = req.body
  const finalEmail = email || (username ? `${username}@practitioners.com` : '')
  if (!username || !finalEmail || !password) {
    return res.status(400).json({ error: 'Username, email, and password are required.' })
  }

  const users = router.db.get('users').value()
  const exists = users.find(u => u.email === finalEmail || u.username === username)
  if (exists) {
    return res.status(409).json({ error: 'An account with this email or username already exists.' })
  }
...
server.post('/api/hospital/:id/deposit', async (req, res) => {
  ...
  let verifiedAmount = depositAmount
  let depositSource = sourceAccount || 'Manual Deposit'

  if (!reference || String(reference).startsWith('DEMO')) {
    // Local demo deposit without Paystack verification
    if (!reference) {
      req.body.reference = `DEMO-${Date.now()}`
    }
  } else {
    try {
      ...
      depositSource = data.data.channel || sourceAccount || 'Paystack Deposit'
    } catch (error) {
  ...
  const txn = {
    ...
    name: depositSource,
    description: `Escrow deposit (Ref: ${reference || req.body.reference})`,
    type: 'DEPOSIT',
    status: 'Completed',
